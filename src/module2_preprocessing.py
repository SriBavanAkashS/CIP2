...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "outputs", "module2_preprocessing")
...
def bandpass_filter(eeg_signal, fs=128):
    ...
    """
    b, a = butter_bandpass(4.0, 45.0, fs)
    y = filtfilt(b, a, eeg_signal, axis=-1)
    return y
...
def save_module2_outputs(D_eeg, D_prep, output_dir=OUTPUT_DIR):
    ...
    print("  Applying Butterworth Bandpass Filter (4.0-45.0 Hz)...")
    ...
    if D_eeg.ndim >= 2:
        ...
        plt.title("Filtered EEG Signal Waveform (After Butterworth Bandpass Filter 4.0-45.0 Hz)", fontsize=14, fontweight='bold')
        plt.xlabel("Time Samples", fontsize=12)
        plt.ylabel("Amplitude (μV)", fontsize=12)
        plt.grid(True, alpha=0.3)
        ...
        plt.subplot(1, 2, 2)
        plt.plot(time_indices, filtered_signal, 'r-', linewidth=1.5)
        plt.title("Filtered EEG Signal\n(After Butterworth Filter 4.0-45.0 Hz)", fontsize=13, fontweight='bold')
        plt.xlabel("Time Samples", fontsize=12)
        plt.ylabel("Amplitude (μV)", fontsize=12)
        plt.grid(True, alpha=0.3)
        ...
        plt.title("Butterworth Bandpass Filter (4.0-45.0 Hz): Overlay Comparison", fontsize=14, fontweight='bold')
        plt.xlabel("Time Samples", fontsize=12)
        plt.ylabel("Amplitude (μV)", fontsize=12)
        ...
        axes[0].axvline(x=4.0, color='green', linestyle='--', linewidth=2, label='Filter Cutoff (4.0 Hz)')
        axes[0].axvline(x=45.0, color='red', linestyle='--', linewidth=2, label='Filter Cutoff (45.0 Hz)')
        ...
        axes[1].axvline(x=4.0, color='green', linestyle='--', linewidth=2, label='Filter Cutoff (4.0 Hz)')
        axes[1].axvline(x=45.0, color='red', linestyle='--', linewidth=2, label='Filter Cutoff (45.0 Hz)')
        axes[1].set_title("Frequency Spectrum: Filtered EEG Signal (After Butterworth Filter 4.0-45.0 Hz)", fontsize=13, fontweight='bold')
...
# --------------------------------------------------
    ...
    # STEP 9: Standardization (Applied to match final training data)
    ...
    summary = {
        ...
        "preprocessing_steps": [
            "1. Bandpass Filtering (Butterworth 4.0–45.0 Hz)",
            "2. Artifact Removal",
...
def preprocess_all_subjects(eeg_data, fs=128):
    ...
    Returns two arrays:
      - Trial outputs: (subjects, trials, 59_segments, channels, window_size)
      - Baseline outputs: (subjects, trials, 2_segments, channels, window_size)
    """

    # Drop the 8 Peripheral Physiological Signals (PPS) to strictly evaluate EEG only
    eeg_data = eeg_data[:, :, :32, :]

    num_subjects, num_trials, _, _ = eeg_data.shape
    all_trials_output = []
    all_baselines_output = []
    ...
    for subj in range(num_subjects):
        ...
        for trial in range(num_trials):
            ...
            # SPLIT INTO BASELINE (3s = 384 samples) AND TRIAL (60s = 7680 samples)
            baseline_signal = cleaned_trial[:, :384]
            trial_signal = cleaned_trial[:, 384:]

            # PROCESS INDEPENDENTLY so z-scores don't mix!
            processed_baseline = eeg_preprocessing_pipeline(baseline_signal, fs, skip_filtering=True, skip_standardization=False)
            processed_trial = eeg_preprocessing_pipeline(trial_signal, fs, skip_filtering=True, skip_standardization=False)

            subject_baselines_out.append(processed_baseline)
            subject_trials_out.append(processed_trial)

        all_baselines_output.append(subject_baselines_out)
        all_trials_output.append(subject_trials_out)

    return np.array(all_trials_output, dtype=np.float32), np.array(all_baselines_output, dtype=np.float32)
...
if __name__ == "__main__":
    ...
    eeg_data_path = os.path.join(PROJECT_ROOT, "outputs", "module1_data_loading", "eeg_data.npy")
    ...
    D_prep_trial, D_prep_baseline = preprocess_all_subjects(eeg_data)

    print("\nFinal preprocessed trial dataset shape:", D_prep_trial.shape)
    print("Final preprocessed baseline shape:", D_prep_baseline.shape)

    # SAVE FULL PREPROCESSED DATASET (FOR TRAINING)
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    np.save(os.path.join(OUTPUT_DIR, "preprocessed_all_subjects.npy"), D_prep_trial)
    np.save(os.path.join(OUTPUT_DIR, "preprocessed_baseline_features.npy"), D_prep_baseline)

    print("\n✓ Full preprocessed dataset saved:")
    print("  preprocessed_all_subjects.npy")
    print("  preprocessed_baseline_features.npy")
    ...
    D_prep_demo = D_prep_trial[0, 0]   # Subject 1, Trial 1
