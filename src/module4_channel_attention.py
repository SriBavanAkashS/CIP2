...
def channel_attention_sequence_block(x_seq, reduction_ratio=8, name_prefix="ca_seq"):
    """
    Apply the Module 4 diagram attention to a *sequence* of feature vectors.

    Input:
      x_seq: (batch, time_steps, feature_dim)

    Output:
      attended_seq: (batch, time_steps, feature_dim)
      weights_seq:  (batch, time_steps, feature_dim)

    This mirrors the diagram per time-step:
      Dense-A + ReLU -> Dense-B + Sigmoid -> Feature Rescaling.

    (GlobalAveragePooling is not needed here because each time step is already a feature vector.)
    """
    ...
    h = TimeDistributed(
        ...
        name=f"{name_prefix}_dense_a_td",
    )(x_seq)
    w = TimeDistributed(
        Dense(feature_dim, activation="sigmoid"),
        name=f"{name_prefix}_dense_b_sigmoid_td",
    )(h)
    attended = Multiply(name=f"{name_prefix}_feature_rescaling")([x_seq, w])
    return attended, w
...
def apply_channel_attention(spatial_features, reduction_ratio=8, return_weights=True):
    ...
    # Try to load the pre-trained attention model saved by extract_pretrained_weights.py
    pretrained_path = os.path.join(PROJECT_ROOT, "outputs", "module4_attention", "attention_model.keras")
    if os.path.exists(pretrained_path):
        from tensorflow.keras.models import load_model
        attention_model = load_model(pretrained_path, compile=False)
        print(f"[Module 4] Loaded pre-trained attention weights from: {pretrained_path}")
    ...
    # Fallback: build a fresh (random) model – only used before extraction is run
