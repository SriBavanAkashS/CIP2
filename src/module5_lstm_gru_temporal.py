...
def temporal_lstm_gru_block(
    ...
    """
    Module 5 block as layers for end-to-end model.

    Input:
      x_seq: (batch, time_steps, feature_dim)
    Output:
      temporal_features: (batch, out_features)
    """
    lstm_h1 = LSTM(lstm_units, return_sequences=True, name=f"{name_prefix}_lstm_1")(x_seq)
    lstm_out = LSTM(lstm_units, return_sequences=False, name=f"{name_prefix}_lstm_2")(lstm_h1)

    gru_h1 = GRU(gru_units, return_sequences=True, name=f"{name_prefix}_gru_1")(x_seq)
    gru_out = GRU(gru_units, return_sequences=False, name=f"{name_prefix}_gru_2")(gru_h1)

    combined = Concatenate(name=f"{name_prefix}_concat")([lstm_out, gru_out])
    temporal_features = Dense(out_features, activation="relu", name=f"{name_prefix}_temporal_dense")(combined)
...
def extract_temporal_features(attended_features, batch_size=64, module2_path=None, window_size=15, step_size=2):
    """
    Extract temporal EEG features using Sliding Window Data Augmentation
    
    Input  : Attention-weighted features from Module 4 (N, 1, 128)
    Output : Temporal feature vectors per augmented window (S, T, NumWindows, 128)
    """
    ...
    for start in range(0, Seg - window_size + 1, step_size):
        windows.append(seq_in[:, start:start+window_size, :])
    
    seq_augmented = np.stack(windows, axis=1)  # (S*T, NumWindows, window_size, F)
    num_windows = seq_augmented.shape[1]
    
    # Flatten trials and windows together for fast batch prediction
    seq_in_model = seq_augmented.reshape(-1, window_size, F)

    # Try to load the pre-trained LSTM-GRU model saved by extract_pretrained_weights.py
    pretrained_path = os.path.join(OUTPUT_DIR, "temporal_model.keras")
    if os.path.exists(pretrained_path):
        from tensorflow.keras.models import load_model as keras_load_model
        # The saved model was built on full 59-seg sequences.
        # We must rebuild for the window_size (15-step) and copy per-layer weights.
        src_model = keras_load_model(pretrained_path, compile=False)
        model = build_lstm_gru_model(input_shape=(window_size, F))
        # Copy weights layer by layer (LSTM/GRU are stateless so timestep count does not affect weights)
        for layer_name in ["lstm_layer_1", "lstm_layer_2", "gru_layer_1", "gru_layer_2", "temporal_dense_layer"]:
            model.get_layer(layer_name).set_weights(src_model.get_layer(layer_name).get_weights())
        print(f"[Module 5] Loaded pre-trained LSTM-GRU weights from: {pretrained_path}")
    else:
        # Fallback: fresh random model (only before extraction has been run)
        model = build_lstm_gru_model(input_shape=(window_size, F))
        print("[Module 5] WARNING: Using randomly-initialized LSTM-GRU. Run extract_pretrained_weights.py first!")

    temporal_features_flat = model.predict(seq_in_model, batch_size=batch_size, verbose=0)  # (S*T * num_windows, 128)
    
    # Reshape back to maintain Subject, Trial, Window hierarchy
    temporal_features = temporal_features_flat.reshape(S, T, num_windows, -1)  # (S, T, NumWindows, 128)

    return temporal_features, model, seq_4d


def save_module5_outputs(temporal_features, temporal_model=None, output_dir=OUTPUT_DIR):
    ...
    if temporal_model is not None:
        model_path = os.path.join(output_dir, "temporal_model.keras")
...
def save_module5_visualizations(seq_4d, temporal_features, output_dir=OUTPUT_DIR, subject_idx=0, trial_idx=0):
    ...
    tf = temporal_features[subject_idx, trial_idx]  # (NumWindows, 128) or (128,)
    if tf.ndim == 2:
        tf = tf[0] # For visualization, just grab the first window's feature vector
...
if __name__ == "__main__":
    ...
    out_path, out_summary = save_module5_outputs(
        temporal_features, temporal_model=temporal_model, output_dir=OUTPUT_DIR
    )
